...
class Woocommerce_Helpers {
	...
	public static function filters_query_args( $settings, $bricks_query_var = [], $element_name = '' ) {
		...
		$query_loop_has_order_settings = ! $is_products_element && isset( $bricks_query_var['orderby'] ); // && isset( $settings['order'] );
		...
		if ( $woo_sort_url !== '' ) {
			...
			$wc_order      = strtoupper( ! empty( $orderby_value[1] ) ? $orderby_value[1] : $products_element_order );
			...
			if ( $is_products_element || ! $query_loop_has_order_settings ) {
				...
				$product_args['orderby'] = $bricks_query_var['orderby'] ?? 'date';
				$product_args['order']   = $bricks_query_var['order'] ?? 'DESC';
